            ...
            <div className="p-6 md:p-10 flex flex-col h-full bg-white dark:bg-gray-800">
                ...
                <div className="flex items-center mb-6">
                    <div className="flex text-yellow-400">
                        ...
                        <Star
                            key={i}
                            className={`h-5 w-5 ${i < Math.round(product.rating.rate) ? 'fill-current' : 'text-gray-200 dark:text-gray-600'}`}
                        />
                        ))}
                    </div>
                    <span className="text-sm font-medium text-gray-500 dark:text-gray-400 ml-3">
                        {product.rating.rate} <span className="text-gray-300 mx-1">•</span> {product.rating.count} reviews
                    </span>
                </div>
                ...
                <div className="mt-auto pt-6 border-t border-gray-100 dark:border-gray-700 flex gap-4">
                    <button
                        ...
                        className={`flex-1 py-4 px-6 rounded-xl flex items-center justify-center gap-3 font-bold text-lg transition-all duration-200 transform active:scale-[0.98] ${
                            ...
                            : 'bg-amber-600 hover:bg-amber-700 dark:bg-amber-500 dark:hover:bg-amber-600 text-white shadow-xl hover:shadow-2xl hover:-translate-y-1'
                        }`}
                    >
                        {isAdded ? (
                            <>
