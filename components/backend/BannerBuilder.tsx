...
interface BannerBuilderProps {
  initialData?: BannerSlide | null;
  onSave: (slide: BannerSlide) => void;
  onCancel: () => void;
}

const BannerBuilder: React.FC<BannerBuilderProps> = ({ initialData, onSave, onCancel }) => {
  ...
  const handleSave = () => {
    // Only image is strictly required now
    if (!formData.image) return;
    
    const newSlide: BannerSlide = {
      id: initialData ? initialData.id : Date.now(),
      title: formData.title || '', // Send empty string if null
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
              ...
              {initialData ? 'Edit Banner' : 'Banner Builder'}
            </h1>
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-800">
              <div className="space-y-4">
                ...
                {/* Image Input */}
                <div>
                   <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Banner Image <span className="text-red-500">*</span></label>
